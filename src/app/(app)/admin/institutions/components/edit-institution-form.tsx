...
import { getInstitutions } from "@/lib/data"; // Assuming getInstitutions can be used for search
...
export function EditInstitutionForm({ onSave, onCancel }: EditInstitutionFormProps) {
  const [searchTerm, setSearchTerm] = React.useState(""); // Can be RBD or name part
  ...
  const handleSearchInstitution = async () => {
    ...
    try {
      ...
      // Simple search: find first match by RBD or name containment
      ...
      if (foundInstitution) {
        ...
        form.reset({
          ...
          contactName: foundInstitution.contactName,
          contactEmail: foundInstitution.contactEmail,
          contactPhone: foundInstitution.contactPhone || "",
          contactRole: foundInstitution.contactRole || "",
        });
        setInstitutionNotFound(false);
      } else {
  ...
  const onSubmit = async (data: InstitutionFormData) => {
    ...
    const institutionToSave: Institution = {
      ...currentInstitution, // Preserve ID and any other fields not in the form (e.g. logo)
      rbd: data.rbd,
      name: data.name,
